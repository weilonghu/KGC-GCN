...
def calc_mrr(embedding, w, test_triplets, all_triplets, hits=[], num_process=4):
    ...
    split_idx = [0] + [i * int(len(test_triplets) / num_process) for i in range(1, num_process - 1)] + [len(test_triplets)]
    processes = [Process(target=_compute_rank, args=(test_triplets[split_idx[i]:split_idx[i + 1]],)) for i in range(num_process)]
    for p in processes:
        p.start()
    for p in processes:
        p.join()
    ...
    # calculate filtered MRR and Hits@n measures
    metrics = {}
